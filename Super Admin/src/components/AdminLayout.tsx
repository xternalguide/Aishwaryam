...
export const AdminLayout: React.FC<AdminLayoutProps> = ({ currentTab, setCurrentTab, children }) => {
  ...
  const navItems: SidebarItem[] = [
    ...
    { id: 'db-updates', label: 'Database Updates', icon: <Database size={18} /> },
  ...
  return (
    <div className="admin-layout">
      ...
      <aside className={`sidebar ${isMobileSidebarOpen ? 'open' : ''}`}>
        <div className="sidebar-logo" style={{ padding: '0 8px', display: 'flex', width: '100%', alignItems: 'center' }}>
          <img
            src="/logo.png"
            alt="Aishwaryam Logo"
            style={{
          ...
          />
          <h1 className="sidebar-title">Aishwaryam</h1>
          <button
            className="btn btn-ghost btn-xs mobile-close-btn"
            onClick={() => setIsMobileSidebarOpen(false)}
            style={{ marginLeft: 'auto', padding: '4px', borderRadius: '50%', color: '#fff' }}
          >
            <X size={18} />
          </button>
        </div>
